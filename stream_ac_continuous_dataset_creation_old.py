...
class StreamAC(nn.Module):
    ...
    def _build_optimizer(self, params, spec: dict, role: str):
        ...
        if opt_name == 'obn':
            return Obn_Optimizer(params, lr=lr, gamma=gamma, lamda=lamda, kappa=kappa, u_trace=u_trace, entrywise_normalization=entrywise_normalization, beta2=beta2)
        if opt_name == 'obnc':
            return ObnC_Optimizer(params, lr=lr, gamma=gamma, lamda=lamda, kappa=kappa, u_trace=u_trace, entrywise_normalization=entrywise_normalization, beta2=beta2)
        if opt_name == 'obnn':
            return ObnN_Optimizer(params, lr=lr, gamma=gamma, lamda=lamda, kappa=kappa, delta_trace=delta_trace, u_trace=u_trace, entrywise_normalization=entrywise_normalization, beta2=beta2)
        if opt_name == 'obtc':
            return ObtC_Optimizer(params, gamma=gamma, lamda=lamda, kappa=kappa, weight_decay=weight_decay, sig_power=sig_power, entrywise_normalization=entrywise_normalization, beta2=beta2, in_trace_sample_scaling=in_trace_sample_scaling)
        if opt_name == 'obtn':
            return ObtN_Optimizer(params, gamma=gamma, lamda=lamda, kappa=kappa, weight_decay=weight_decay, sig_power=sig_power, delta_trace=delta_trace, entrywise_normalization=entrywise_normalization, beta2=beta2, in_trace_sample_scaling=in_trace_sample_scaling)
        if opt_name == 'obt':
            return Obt_Optimizer(params, gamma=gamma, lamda=lamda, kappa=kappa, weight_decay=weight_decay, sig_power=sig_power, delta_clip=delta_clip, delta_norm=delta_norm, entrywise_normalization=entrywise_normalization, beta2=beta2, in_trace_sample_scaling=in_trace_sample_scaling)
        if opt_name == 'obtnnz':
            return Obtnnz_Optimizer(params, gamma=gamma, lamda=lamda, kappa=kappa, weight_decay=weight_decay, delta_clip=delta_clip, delta_norm=delta_norm, u_trace=u_trace, entrywise_normalization=entrywise_normalization, beta2=beta2)
        if opt_name == 'obgdn':
            return ObGDN_Optimizer(params, lr=lr, gamma=gamma, lamda=lamda, kappa=kappa, delta_clip=delta_clip, delta_norm=delta_norm)
        if opt_name == 'obgdm':
            return ObGDm_Optimizer(params, lr=lr, gamma=gamma, lamda=lamda, kappa=kappa, momentum=momentum)
        if opt_name == 'obtcm':
            return ObtCm_Optimizer(params, gamma=gamma, lamda=lamda, kappa=kappa, weight_decay=weight_decay, sig_power=sig_power, momentum=momentum, entrywise_normalization=entrywise_normalization, beta2=beta2, in_trace_sample_scaling=in_trace_sample_scaling)
    ...
    def update_params(self, s, a, r, s_prime, done):
        ...
        # We need gradients for both policy and critic, so we compute them
        # before zeroing grads
        ...
        # Let's use standard REINFORCE-style objective
        # Objective = log_prob * (TD_error) + entropy_coeff * Entropy
        # We use .backward() on the loss, which is the negative of the objective
        loss_policy = -log_prob_pi - self.entropy_coeff * entropy_pi
        
        loss_policy.backward()
        info_policy = self.optimizer_policy.step(delta_policy, reset=done)

        # Return minimal info, or None
        return {'policy': info_policy, 'critic': info_critic}

def main(env_name, seed, total_steps, max_time, policy_spec, critic_spec, save_dir, run_name_suffix):
    ...
    # ---- Generate Run Name ----
    run_name = (f'{env_name}____-Policy_{spec_to_name(policy_spec)}____-Critic_{spec_to_name(critic_spec)}' + 
    ...
    for t in range(1, int(total_steps) + 1):
        ...
        # *** r_scaled is the reward from the ScaleReward wrapper ***
        s_prime, r_scaled, terminated, truncated, info = env.step(a)

        # *** r_actual is the true, unscaled reward from the env ***
        # Use r_scaled as default if 'reward_immediate' isn't in info
        r_actual = info.get('reward_immediate', r_scaled)
        done = terminated or truncated

        # *** MODIFIED: Store the 6-item tuple as requested ***
        dataset.append((s, a, r_scaled, r_actual, s_prime, done))

        # Update the agent (uses r_scaled for its own learning)
    ...
    # --- Save the dataset ---
    print(f"\nSaving dataset with {len(dataset)} transitions...")
    if save_dir == 'none':
        ...
        try:
            ...
            # Using the save path from your provided code
            save_path = os.path.join(save_dir, f"{env_name.split('-')[0]}_seed{seed}.pkl")
            with open(save_path, "wb") as f:
                save_data = {
                    'dataset': dataset,
                    'env_name': env_name,
                    'seed': seed,
                    'data_set_run_name': run_name
                }
                pickle.dump(save_data, f)
            print(f"Save complete to: {save_path}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--policy_in_trace_sample_scaling', type=str, default='True', choices=['True', 'False'])
    ...
    parser.add_argument('--critic_in_trace_sample_scaling', type=str, default='True', choices=['True', 'False'])
    ...
    def build_spec(kind, args, required_optimizer_params) -> dict:
        ...
        # Handle cases where an optimizer might not be in the list (e.g., 'none')
