...
class Obtnnz(torch.optim.Optimizer): # Ob with trace and no norm z
    ...
    def step(self, delta, reset=False):
        ...
        info = {'delta':delta, 'delta_clipped_normed':safe_delta, 'abs_delta':abs(delta)}
        return info
...
class Obt(torch.optim.Optimizer): # same as Obn but also has delta_clipping
    ...
    def step(self, delta, reset=False):
        ...
        info = {'M':dot_product, 'clipped_step_size':step_size, 'delta':delta, 'delta_clipped_normed':safe_delta, 'abs_delta':abs(delta), 'norm1_eligibility_trace':z_sum}
        return info
