...
def main(dataset_path, seed, observer_spec, logging_spec, debug=False):
    ...
    try:
        ...
        data_seed = data.get('seed', 'unknown')
        print(f"Loaded {len(dataset)} transitions from env '{env_name}' (data seed: {data_seed}).")
    ...
    try:
        s_sample, _, _, _, _, _ = dataset[0]
        n_obs = np.array(s_sample).shape[0]
        print(f"Inferred n_obs = {n_obs}")
    except Exception as e:
        print(f"Error inferring n_obs from data (expected 6 items): {e}")
        return

    agent = OfflineObserver(n_obs=n_obs, observer_spec=observer_spec)
    gamma_obs = agent.gamma_observer
    ...
    for t, (s, a, r_scaled, r_actual, s_prime, done) in enumerate(dataset):
        #print(f's {s}\na {a}\nr_scaled {r_scaled}\nr_actual {r_actual}\ns_prime {s_prime}\ndone {s}\n')
        if is_new_episode:
        ...
        if logging_this_episode and not agent.is_monte_carlo and log_freq > 0:
            ...
            if payload_for_this_step:
                # --- CHANGE 2: Log against relative step, not global step 't' ---
        ...
        if done or (t == total_steps - 1):
            ...
            if logging_this_episode and agent.is_monte_carlo and log_freq > 0:
                ...
                # start_step_of_this_ep = t - ep_len + 1 # <--- Not needed

                for i in range(ep_len):
                    # --- CHANGE 3: Log against relative step 'i', not global step ---
                    relative_step = i
                    payload_for_this_step = {
            ...
            # This part is unchanged and correctly uses the global step 't'
            if agent.is_monte_carlo:
                log_payload = {
            ...
            log_payload['step'] = int(t + 1) # Group 1 logs still use global step
...
if __name__ == '__main__':
    ...
    # Core Arguments
    parser.add_argument('--dataset_path', type=str, default='/Users/arsalan/Desktop/Codes/StreamX/offline_dataset/ObGD_ObGD_lam_0.8/Ant_seed0.pkl', help='Path to the offline dataset .pkl file to load.')
