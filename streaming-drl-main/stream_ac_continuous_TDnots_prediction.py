...
from optim import TDNots as Optimizer
...
class StreamAC(nn.Module):
    def __init__(self, n_obs=11, n_actions=3, hidden_size=128, lr=1.0, gamma=0.99, lamda=0.8, kappa_policy=3.0, kappa_value=2.0, eta_value=1.0):
        ...
        self.optimizer_value = Optimizer(self.value_net.parameters(), lr=lr, gamma=gamma, lamda=lamda, kappa=kappa_value, eta=0.0)
        self.observer_value = Optimizer(self.value_net_observer.parameters(), lr=lr, gamma=gamma, lamda=lamda, kappa=kappa_value, eta=eta_value)
    def pi(self, x):
    ...
    def update_params(self, s, a, r, s_prime, done, entropy_coeff, overshooting_info=False):
        ...
        v_prime_grads = self.compute_grad_v(self.optimizer_value, v_prime)
        ...
        pol_info = self.optimizer_policy.policy_step(delta.item(), reset=done)
        val_info_cntrl = self.optimizer_value.value_step(delta.item(), reset=done, v_prime_grads=v_prime_grads)

        # observer update:
        v_s_obs, v_prime_obs = self.v_observer(s), self.v_observer(s_prime)
        v_prime_obs_grads = self.compute_grad_v(self.observer_value, v_prime_obs)
        (-v_s_obs).backward()
        delta_obs = r + self.gamma * v_prime_obs * done_mask - v_s_obs
        val_info = self.observer_value.value_step(delta_obs.item(), reset=done, v_prime_grads=v_prime_obs_grads)
...
def main(env_name, seed, lr, gamma, lamda, total_steps, entropy_coeff, kappa_policy, kappa_value, eta_value, debug, overshooting_info, render=False,
    ...
    env = AddTimeInfo(env)
    agent = StreamAC(n_obs=env.observation_space.shape[0], n_actions=env.action_space.shape[0], lr=lr, gamma=gamma, lamda=lamda, kappa_policy=kappa_policy, kappa_value=kappa_value, eta_value=eta_value)
    if debug:
    ...
    logger = get_logger(
        ...
        log_dir=log_dir,
        project=project,
        run_name= f"{env.spec.id}_TDnots_eta_{eta_value}_kappa_{kappa_value}_seed{seed}__ent_{entropy_coeff}"+ run_name,
        config=config
    )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--project', type=str, default='StreamX_Prediction_', help='WandB project (if backend=wandb)')
    parser.add_argument('--run_name', type=str, default='deltabar1___nomin1', help='Run name for logger') # __sqrt_coeff

    args = parser.parse_args()
    
    main(args.env_name, args.seed, args.lr, args.gamma, args.lamda, args.total_steps, args.entropy_coeff,
         args.kappa_policy, args.kappa_value, args.eta_value, args.debug, args.overshooting_info, args.render,
