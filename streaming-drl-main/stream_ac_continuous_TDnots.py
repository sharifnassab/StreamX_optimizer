...
class StreamAC(nn.Module):
    def __init__(self, n_obs=11, n_actions=3, hidden_size=128, lr=1.0, gamma=0.99, lamda=0.8, kappa_policy=3.0, kappa_value=2.0, eta_value=1.0):
        ...
        self.optimizer_value = Optimizer(self.value_net.parameters(), lr=lr, gamma=gamma, lamda=lamda, kappa=kappa_value, eta=eta_value)
    ...
    def update_params(self, s, a, r, s_prime, done, entropy_coeff, overshooting_info=False):
        ...
        value_params_grouped = [list(g["params"]) for g in self.optimizer_value.param_groups]
        value_params = [p for grp in value_params_grouped for p in grp]
        v_prime_grads_flat = torch.autograd.grad(v_prime.sum(), value_params)
        it = iter(v_prime_grads_flat)
        v_prime_grads = [[next(it) for _ in grp] for grp in value_params_grouped]
        ...
        val_info = self.optimizer_value.value_step(delta.item(), reset=done, v_prime_grads=v_prime_grads)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--eta_value', type=float, default=0.8)
