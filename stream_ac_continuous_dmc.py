...
import shimmy # for DMC envs
from dm_control import suite  
#gym.register_envs(shimmy)  # explicit registration for external envs
...
def main(env_name, seed, total_steps, max_time, policy_spec, critic_spec, observer_spec={'optimizer':'none'}, logging_spec={'backend':'none'}, debug=False, render=False):
    ...
    if env_name.startswith("dm_control/"):
        spec = env_name.removeprefix("dm_control/").removesuffix("-v0")
        domain, task = spec.split("-", 1)
        env = shimmy.DmControlCompatibilityV0(
            suite.load(domain_name=domain, task_name=task),
            render_mode="human" if render else None,
        )
        env.unwrapped.spec = gym.envs.registration.EnvSpec(
                                    id=env_name,
                                    entry_point=None,
